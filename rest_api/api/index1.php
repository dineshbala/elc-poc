...
$note_array = array();
    ...
    try {
        ...
        $stmt = $db->prepare($sql);
        $stmt->execute();
        $usersList_array = $stmt->fetchAll(PDO::FETCH_OBJ);
        $usersList_array['skus'] = array();
        
        $sql1 = "SELECT * FROM tbl_product_skus";
        $stmt1 = $db->prepare($sql1);
        $stmt1->execute();
        $user_array = $stmt1->fetchAll(PDO::FETCH_OBJ);
        array_push($usersList_array['skus'],$user_array);
        
        array_push($note_array,$usersList_array);
